...
// Extend Window for Google Maps callback and namespace
declare global {
  interface Window {
...
export default function BookDemoForm(): JSX.Element {
  ...
  return (
    <>
      ...
      <section id="contactus" className="relative bg-gradient-to-r from-blue-50 via-indigo-100 to-blue-200 pt-16 pb-24" aria-labelledby="contactus-title">
        <Toaster position="top-right" />

        <motion.div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-indigo-200 to-blue-300 opacity-40 rounded-full filter blur-3xl" animate={{ x: [0, 40, 0], y: [0, -40, 0] }} transition={{ duration: 18, repeat: Infinity }} />
        <motion.div className="absolute bottom-10 right-1/4 w-80 h-80 bg-gradient-to-br from-pink-100 to-purple-200 opacity-40 rounded-full filter blur-2xl" animate={{ x: [0, -30, 0], y: [0, 30, 0] }} transition={{ duration: 14, repeat: Infinity }} />

        <h2 id="contactus-title" className="text-3xl sm:text-4xl font-extrabold text-gray-900 text-center">
          Contact Us for a Free <span className="text-blue-600">Demo</span>
        </h2>
        <p className="text-xs text-gray-700 text-center mb-12">
          Give us a call on{' '}
          <a href="tel:+919977858817" className="font-medium text-blue-600 hover:underline">
            +91 99778 58817
          </a>{' '}
              ...
              <Controller control={control} name="requirement" render={({ field }) => (
                <Listbox as="div" {...field}>
                  ...
                  <div className="relative mt-1">
                    <Listbox.Button className="w-full pl-3 pr-10 py-3 text-left border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><span className="block truncate">{field.value}</span><ChevronUpDownIcon className="absolute right-3 top-1/2 w-5 h-5 text-gray-400 -translate-y-1/2" /></Listbox.Button>
                      ...
                      <Listbox.Options className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto focus:outline-none">
                        {requirementOptions.map((option) => (
                          <Listbox.Option key={option} value={option} className={({ active }) => `cursor-pointer select-none px-4 py-2 ${active ? 'bg-blue-100' : ''}`}>
                            {({ selected }) => <div className="flex items-center justify-between"><span>{option}</span>{selected && <CheckIcon className="w-5 h-5 text-blue-600" />}</div>}
                          </Listbox.Option>
                        ))}
                      </Listbox.Options>
                    </Transition>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}>
            <div className="bg-white shadow-lg rounded-2xl overflow-hidden">
              ...
              <div className="p-6 text-center">
                ...
                <Link href="https://g.co/kgs/v7D5HuZ" target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow hover:bg-blue-700 transition">
                  View on Google Maps
                </Link>
              </div>
            </div>
