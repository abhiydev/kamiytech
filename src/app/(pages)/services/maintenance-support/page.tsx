...
import { FaBug, FaTachometerAlt, FaShieldAlt, FaPlusCircle, FaSyncAlt, FaDatabase, FaHeadset } from 'react-icons/fa';
...
const MaintenanceSupport: React.FC = () => (
  <>
    <Navbar />
    <Head>
      <title>Maintenance & Support – KamiyTech</title>
      <meta name="description" content="Comprehensive post-launch maintenance & support services from KamiyTech." />
    </Head>

    <main className="bg-gray-50 py-16">
      <div className="max-w-5xl mx-auto px-4">
        <header className="text-center mb-12">
          <h1 className="text-5xl font-extrabold text-gray-900">Maintenance & Support</h1>
          <p className="mt-4 text-lg text-gray-600">Ensuring your digital products stay secure, optimized, and reliable.</p>
        </header>
          ...
          {services.map((svc, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-lg transition-shadow duration-300">
              <div className="flex items-center mb-4">
                ...
                <h2 className="ml-4 text-xl font-semibold text-gray-800">{svc.title}</h2>
              </div>
        ...
        <section className="mt-16 bg-white p-8 rounded-2xl shadow-md">
          <h2 className="text-3xl font-semibold text-gray-800 mb-6">Available Maintenance Plans</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            {['3-Month Plan', '6-Month Plan', '1-Year Plan'].map((plan, i) => (
              <div key={i} className="p-6 border border-gray-200 rounded-xl hover:border-blue-500 transition-colors">
                <h3 className="text-xl font-medium text-gray-800 mb-2">{plan}</h3>
                <p className="text-gray-600">Full-featured support services for the duration of the plan.</p>
              </div>
            ))}
          </div>
        </section>
